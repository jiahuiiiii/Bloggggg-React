...
function Post({
  ...
  return (
    <motion.div
      ...
      className="flex flex-row p-8 hover:shadow-2xl bg-amber-100 bg items-center justify-between border border-neutral border-1  "
    >
      <div>
        <div className="text-4xl font-semibold tracking-wide">{item.name}</div>
        <div className="mt-1">{moment(item.date).format('LLL')}</div>
        <div className="text-neutral-500 text-xl mt-2">
          {(() => {
        ...
        <Link
          to={`/article/${item._id}`}
          className="flex gap-2 mt-2 transition-all hover:gap-4 items-center"
        >
